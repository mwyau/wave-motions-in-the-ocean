...
A scientifically “better” redraw is not automatically a faithful reconstruction. If the source figure, nearby equation, and physical interpretation disagree, preserve the source representation and record a `pending-human-approval` erratum. Only an explicitly `human-approved` erratum authorizes a substantive corrected redraw.
