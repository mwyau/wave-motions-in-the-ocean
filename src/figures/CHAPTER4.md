...
- **Representation:** vector
- **Equation check:** ai-checked
- **Scientific check:** The vector preserves the transverse `u`/`k` construction, `w=u sin(theta)`, and the source's displayed `u_t=-g p sin(theta)/rho_0` relation. The nearby derivation gives the density-perturbation form; see `ERRATA.md`, printed p.69. The source remains unchanged pending human review.
...
- **Representation:** vector
- **Equation check:** ai-checked
- **Scientific check:** The subcritical construction uses common `R=1.25`, `a=0.32`; directions are `+/-atan(R)` and arrow lengths use `q=(1+aR)/(1-aR)=2.333333`. It makes no supercritical magnitude claim. See `ERRATA.md`, printed p.90.
