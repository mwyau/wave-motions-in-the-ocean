...
| 4 | 91 | `ChapmanRizzoli4.pdf` | 28 | **Partial** | **source-pdf** — deliberately retain the multi-slope reflection sketch. The matching calculation is checked: `k_r/k_i=(1+aR)/(1-aR)` is the signed component relation, so the corresponding magnitude ratio is `|(1+aR)/(1-aR)|`, and `aR=1` is the inviscid singular limit described in the text. The exact historical arrow/phase convention for the supercritical `aR>1` sketches remains source-specific and is not normalized or redrawn; this intentional unresolved detail keeps the row `Partial`. |
...
| 6 | 159 | `ChapmanRizzoli6.pdf` | 12 | **Validated** | **source-pdf** — deliberately retain the historical full modal dispersion diagram. Along each matched mode `tan(kL)=k/(ell-b)` makes `k=k_n(ell)`, so the modal extremum obeys `k^2+b^2-ell^2=2 ell k dk_n/dell`, not the source fixed-`k` condition. For `b=L=f=1`, independently solved maxima for modes 1--4 are `(ell,sigma)=(-2.42327665,0.36806052)`, `(-5.25577519,0.17665842)`, `(-8.30307686,0.11437907)`, and `(-11.40045725,0.08434122)`, preserving one maximum per branch and decreasing peak frequency with mode number. The source fixed-`k` maximum is a documented historical equation error; retaining the source art is intentional. |
