% ===== merged from chapter01-p001-008.tex; source-page comments retained =====
% Wave Motions in the Ocean: Myrl's view
% Verbatim reconstruction: Chapter 1, printed pages 1--8.
% Source authority: ChapmanRizzoli0_2.pdf, physical pages 11--18.
...
\]
where $\Im$ refers to the imaginary part of the expression. However, it is often much
...
\par\smallskip
\begin{center}
\input{figures/ch01-p004-phase-speed.tikz}
\end{center}
\smallskip
...
\]
It is directed along $\vec{k}$. Note that the speed of phase plane intersection with the
...
cases (a)--(d), the phase speed $c=\sigma/|\vec{k}|$ is independent of wavelength,
frequency or direction. Such waves are nondispersive or dispersionless because all waves
(for each case individually) travel with the same speed. In case (e), the phase speed $c$
is dependent upon the wavelength and the direction, so these waves are dispersive. As we
will see, this basically means that a group of such
...
\par\smallskip
\begin{center}
\input{figures/ch01-p008-wave-packet.tikz}
\end{center}
\smallskip
...
\par\smallskip
\begin{center}
\input{figures/ch01-p008-spectrum.tikz}
\end{center}
\smallskip

The modulated plane wave is said to be a `narrow band signal'.

% ===== merged from chapter01-p009-017.tex; source-page comments retained =====
% Wave Motions in the Ocean: Myrl's view
% Verbatim reconstruction: Chapter 1, printed pages 9--17.
% Source authority: ChapmanRizzoli0_2.pdf, physical pages 19--27.
% Source wording is retained except for corrections explicitly logged in ERRATA.md.
% Original diagrams are cropped directly from the committed source PDF.
...
$\Delta a/a\ll1$ and $\Delta\Theta/\Theta\ll1$ over $|\vec{k}|^{-1}$ and $N^{-1}$.
