...
EQUATION_RENDER_CONFIG = {
    "source": ("source-pdf", "pdf-raster;dpi=300;recorded-page-crop"),
    "mathjax": (
        "chromium-mathjax",
        "mathjax=3.2.2;chtml;scale=2;viewport=content;background=white",
    ),
    "mathml": (
...
def _manifest_text(
    ...
    return (
        "\n".join(
            (
                "# Generated by scripts/publication.py",
...
def _equation_asset_input_metadata(
    ...
    renderer, renderer_config = EQUATION_RENDER_CONFIG[kind]
    metadata = {
...
EQUATION_ASSET_RENDER_METADATA_KEYS = frozenset(
    {
...
def summarize_equation_asset_errors(errors: Iterable[str]) -> list[str]:
    """Collapse repeated renderer-configuration failures for concise full checks."""
    summarized: list[str] = []
    ...
    for error in errors:
        ...
        if (
            match is None
            or match.group("key") not in EQUATION_ASSET_RENDER_METADATA_KEYS
        ):
